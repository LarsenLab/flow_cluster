...
##### Step 6: Downsample and Reduce Dimensions (UMAP)
...
adapt_clusters_cd19_pos_umap <- adapt_clusters_cd19_pos_down |>
    tof_reduce_umap(num_comp = 3)

adapt_clusters_cd19_pos_full <- adapt_clusters_cd19_pos_down |> 
    bind_cols(adapt_clusters_cd19_pos_umap)

adapt_clusters_cd19_pos_umap <- adapt_clusters_cd19_pos_full
# 
# 
...
```{r}
adapt_clusters_cd19_pos_umap |>
    ggplot(aes(x = .umap1, y = .umap2, color = .flowsom_metacluster)) +
    geom_point() +
    theme_bw() +
    labs(fill = NULL)+ 
scale_color_manual(values = colorRampPalette(brewer.pal(n = 9, name = "Spectral"))(length(unique(adapt_clusters_cd19_pos_umap$.flowsom_metacluster)))) +
  tmp_theme + 
  theme(aspect.ratio = 1) + 
  guides(color = "none")

adapt_clusters_cd19_pos_umap |>
  ggplot(aes(x = .umap1, y = .umap2, color = as.factor(.flowsom_metacluster))) +  # Convert to factor if discrete
  geom_point() +
  theme_bw() +
  labs(fill = NULL) +
  facet_wrap(~ visit) +
...
adapt_clusters_long <- adapt_clusters_cd19_pos_umap |>
  select(cd19:bcma,.flowsom_metacluster:.umap2,visit)|>
...
ggplot(adapt_clusters_long, aes(x = .umap1, y = .umap2, color = expression)) +
  geom_point() +
  theme_bw() +
  scale_color_gradientn(values = c(1, 0), na.value = "darkred",
colours = colorRampPalette((brewer.pal(n = 9, name = "Spectral")))(50)) +
...
cluster_labels <- adapt_clusters_long |>
  ...
  summarise(
    label_x = median(.umap1, na.rm = TRUE),
    label_y = median(.umap2, na.rm = TRUE),
    .groups = 'drop'
  )


ggplot(adapt_clusters_long, aes(x = .umap1, y = .umap2, color = .flowsom_metacluster)) +
  geom_point() +  
  #facet_wrap(~ marker, scales = "free") +
  geom_text(
    data = cluster_labels,
...
p <- plot_ly(adapt_clusters_cd19_pos_umap, x=~.umap1, y=~.umap2, 
z=~.umap3, color=~.flowsom_metacluster) 
# %>%
...
adapt_clusters_cd19_neg <- adapt |> filter(population == "CD19-") |>
    tof_cluster(method = "flowsom", cluster_cols = all_of(clust_cols_neg), num_metaclusters = 8) 
```
...
adapt_clusters_cd19_neg_umap <- adapt_clusters_cd19_neg_down |>
    tof_reduce_umap(num_comp = 3)

adapt_clusters_cd19_neg_full <- adapt_clusters_cd19_neg_down |> 
    bind_cols(adapt_clusters_cd19_neg_umap)

adapt_clusters_cd19_neg_umap <- adapt_clusters_cd19_neg_full
```
...
num_clusters <- length(unique(adapt_clusters_cd19_neg_umap$.flowsom_metacluster))

# Ensure color palette has enough colors
adapt_clusters_cd19_neg_umap |>
  ggplot(aes(x = .umap1, y = .umap2, color = .flowsom_metacluster)) +
  geom_point() +
  theme_bw() +
  labs(fill = NULL) + 
...
adapt_clusters_long <- adapt_clusters_cd19_neg_umap |>
  select(all_of(clust_cols_neg),.flowsom_metacluster:.umap2,visit)|>
...
p <- plot_ly(adapt_clusters_cd19_neg_umap, x=~.umap1, y=~.umap2, 
z=~.umap3, color=~.flowsom_metacluster) 
# %>%
#      add_markers(size=1.5)
print(p)
