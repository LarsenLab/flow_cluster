...
adapt <- adapt |>
  tof_preprocess(
    channel_cols = all_of(channel_cols),  # Use the selected column names
    transform_fun = scale
  ) |>
  tof_preprocess(
...
adapt_clusters_cd19_pos <- adapt |> filter(population == "CD19+") |>
    tof_cluster(method = "flowsom", cluster_cols = all_of(clust_cols), 
                num_metaclusters = 8)
...
p <- plot_ly(adapt_clusters_long, x=~.tsne1, y=~.tsne2, 
