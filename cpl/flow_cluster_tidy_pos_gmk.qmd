...
fcs_data <- tof_tbl_numeric |>
    ...
    mutate(visit = str_extract(time_point, "V.+(?=_)"))  |> 
...
```{r}
expr_matrix <- as.matrix(fcs_data)

seurat_obj <- CreateSeuratObject(counts = expr_matrix)

expr_matrix_1 <- crowbar::to_sparse_matrix(expr_matrix)

library(Matrix)

tof_tbl_numeric <- fcs_data %>% select(where(is.numeric))

dgCMatrix_obj <- as(as.matrix(tof_tbl_numeric), "dgCMatrix") 

seurat_obj <- CreateSeuratObject(counts = dgCMatrix_obj)

counts_df <- as_tibble(as.data.frame(as.matrix(seurat_obj@assays$RNA@layers$counts)), rownames = "gene")

# Extract metadata and convert to tibble
metadata_tbl <- as_tibble(seurat_obj@meta.data, rownames = "cell")

# If you want to join counts with metadata (long format), pivot the counts data
# Here’s an example of merging counts with metadata
tidy_counts <- counts_df %>%
  pivot_longer(-gene, names_to = "cell", values_to = "expression") %>%
  inner_join(metadata_tbl, by = "cell")
```
...
adapt_pos_seurat <- adapt_pos |> 
    as_seurat()

class(adapt_pos)
class(adapt_pos_seurat)
```
...
# before preprocessing
adapt_pos |>
    select(`CD19|FJComp-BUV C-A`,
           `CD38|FJComp-Blue B-A`) |>
    head()
```
...
adapt_pos |>
    ...
    # scale_y_continuous(limits = c(NA, 4)) +
    theme_bw() + 
    facet_wrap(vars(visit, population))
...
adapt_pos |>
    ...
    # geom_point(alpha = 0.5) +
    # geom_contour() +
    geom_density_2d_filled(bins = 8, colour = "blue") +
    # scale_x_continuous(limits = c(NA, 1.5)) +
    # scale_y_continuous(limits = c(NA, 4)) +
    theme_bw()
