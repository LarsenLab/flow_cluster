...
adapt_clusters_cd19_neg <- adapt |> filter(population == "CD19-") |>
    tof_cluster(method = "flowsom", cluster_cols = all_of(clust_cols_neg), num_metaclusters = 6)
```
...
adapt_clusters_long <- adapt_clusters_cd19_neg_tsne |>
  select(clust_cols_neg,.flowsom_metacluster:.tsne2,visit)|>
  pivot_longer(cols=clust_cols_neg,names_to = "marker", values_to = "expression")|> 
    mutate(.flowsom_metacluster = as.factor(.flowsom_metacluster),
           marker = as.factor(marker),
           visit=as.factor(visit)) 
