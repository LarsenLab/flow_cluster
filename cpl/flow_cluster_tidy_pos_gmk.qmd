...
fviz_nbclust(df_pos, kmeans, method = "wss", k.max = 10,
  nboot = 100) +
  ggtitle("Elbow Plot for Optimal Number of Clusters") +
  theme_minimal()
```
...
fviz_nbclust(df_neg, kmeans, method = "wss", k.max = 10,
  nboot = 100) +
  ggtitle("Elbow Plot for Optimal Number of Clusters") +
  theme_minimal()
```
...
adapt_clusters_cd19_neg <- adapt |> filter(population == "CD19-") |>
    tof_cluster(method = "flowsom", cluster_cols = all_of(clust_cols_neg), num_metaclusters = 7) 
```
...
p <- plot_ly(adapt_clusters_long, 
             x=~.tsne1, 
             y=~.tsne2, 
             z=~expression, 
         color=~.flowsom_metacluster) 
# %>%
#      add_markers(size=1.5)
print(p)
...
\<\<\<\<\<\<\< HEAD Here is the reference manual for additional information: \[tidytof:A user-friendly framework for interactive and reproducible cytometry data analysis\] (https://keyes-timothy.github.io/tidytof/articles/modeling.html)

```{r}
adapt_clusters_cd19_neg_umap <- adapt_clusters_cd19_neg |>
    tof_downsample(method = "constant", group_cols = visit, num_cells = 1000) |>
...
p <- plot_ly(adapt_clusters_long, x=~.umap1, y=~.umap2, 
z=~expression, color=~.flowsom_metacluster) 
# %>%
#      add_markers(size=1.5)
print(p)
